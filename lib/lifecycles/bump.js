...
async function Bump (args, version) {
  ...
  await runLifecycle({ hookName: 'prerelease', args, version })
  const stdout = await runLifecycle({ hookName: 'prebump', args, version })
  ...
  await runLifecycle({ hookName: 'postbump', args, version, newVersion })
  return newVersion
}
