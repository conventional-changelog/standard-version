...
async function Bump (args, version) {
  ...
  await runLifecycleHook({hookName:'prerelease',args,version})
  const stdout = await runLifecycleHook({hookName:'prebump',args,version})
  ...
  await runLifecycleHook({hookName:'postbump',args,version,newVersion})
  return newVersion
}
