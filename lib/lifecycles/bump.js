...
function Bump (args, version, pkg) {
  ...
  return runLifecycleScript(args, 'prerelease')
    ...
    .then((release) => {
      if (!args.firstRelease) {
        ...
        updateConfigs(args, newVersion, pkg)
      } else {
 ...
 * @return void
 */
function updateConfigs (args, newVersion, pkg) {
  ...
  args.bumpFiles.forEach(function (bumpFile) {
    ...
    try {
      ...
      checkpoint(
        ...
        [updater.updater.readVersion(contents, pkg.name), newVersion]
      )
      writeFile(
        args,
        configPath,
        updater.updater.writeVersion(contents, newVersion, pkg.name)
      )
