...
async function Bump (args, version) {
  ...
  await runLifecycle({ hookName: 'prerelease', args, context: { version } })
  const stdout = await runLifecycle({ hookName: 'prebump', args, context: { version } })
  ...
  await runLifecycle({ hookName: 'postbump', args, context: { version, newVersion } })
  return newVersion
}
