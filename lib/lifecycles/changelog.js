...
function getTemplates(templates) {
  ...
  var partialTemplates = [ 
    { option: 'commitPartial', template: templates.commit },
    { option: 'headerPartial', template: templates.header },
    { option: 'footerPartial', template: templates.footer }
  ];
  for (i = 0; i < partialTemplates.length; ++i) {
    let templatePath = ResolveTemplatePath(partialTemplates[i].template)
    if (templatePath != undefined) {
      writerOptions[partialTemplates[i].option] = templatePath;
    }
  };
...
function outputChangelog (args, newVersion) {
  return new Promise((resolve, reject) => {
    ...
    const writerOptions = getTemplates(args.templates)
