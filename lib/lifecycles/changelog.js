...
function outputChangelog (args, newVersion) {
  return new Promise((resolve, reject) => {
    ...
    // the header must not match the regex used to find
    // the last release.
    if (header.search(START_OF_LAST_RELEASE_PATTERN)) {
      return reject(Error(`the changelogHeader provided must not match ${START_OF_LAST_RELEASE_PATTERN}`))
    }
