...
function outputChangelog (args, newVersion) {
  return new Promise((resolve, reject) => {
    ...
    let oldContent = args.dryRun ? '' : fs.readFileSync(path.join(args.path || process.cwd(), args.infile), 'utf-8')
    ...
    const changelogStream = conventionalChangelog({
      ...
      pkg: { path: args.path },
      path: args.path
    }, context, { merges: null, path: args.path }, null, null, { cwd: args.path })
...
function createIfMissing (args) {
  try {
    accessSync(path.join(args.path || process.cwd(), args.infile), fs.F_OK)
  } catch (err) {
    if (err.code === 'ENOENT') {
